...
class NodeController extends Controller
{
    ...
    public function actionSave()
    {
        ...
        if ($treeNodeModify) {
			...
			if(isset($node->banner) && $node->banner != '')
				$banner_val = $node->banner;
			
			if(isset($node->image) && $node->image != '')
				$image_val = $node->image;
        ...
        if ($node->save()) {
			...
			if(isset($node->image) && isset($node->banner)){
				$node->image = $image_val;
				$node->banner = $banner_val;
			}
			if($image)
			{
				$name = str_replace(' ','-',strtolower($node->name.'_'.$node->id));
			...
			if($banner)
			{
				$name = str_replace(' ','-',strtolower($node->name.'_banner_'.$node->id));
				$size = Yii::$app->params['folders']['size'];
				$main_folder = "category";
				$image_name= $this->uploadImage($banner,$name,$main_folder,$size);
				$node->banner = $image_name;
			}
			if($banner || $image)
				$node->save();
